...
Section
  ...
  WriteRegStr HKCU "Software\Classes\bitsend" "" "URL:bitsend Protocol"
  WriteRegStr HKCU "Software\Classes\bitsend" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitsend" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitsend\shell\open\command" "" "$\"$INSTDIR\${PRODUCT_NAME}-${PRODUCT_VERSION}.exe$\" $\"%1$\""
